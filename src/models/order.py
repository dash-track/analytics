...
from src.services.redis.redis_interface import RedisInterface

class Order(RedisInterface):
    """
    ...
    def __init__(self, restaurant_name="", amount_spent_total="", date_of_order="", items={}):
        """
    ...
    def inject_redis_client(self, redis_client):
        """
    ...
    def save(self):
        ...
        """
        assert self._redis_client is not None and isinstance(self._redis_client, redis.Redis), "Redis client not injected"
    ...
    def load(self, key, redis_client):
        ...
        """
        assert isinstance(redis_client, redis.Redis), "Redis client not passed to load method"
        order_details = redis_client.hgetall(f'order:{key}')
        order_details = {k.decode('utf-8'): v.decode('utf-8') for k, v in order_details.items()}
        ...
        for item_key in items_keys:
            ...
            item_details = {k.decode('utf-8'): v.decode('utf-8') for k, v in item_details.items()}
